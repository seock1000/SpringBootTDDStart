...
- [x] continuationToken에 빈 문자열이 지정되면 첫번째 페이지를 반환한다
...
- 테스트 클래스에 @BeforeEach, @BeforeAll 어노테이션을 사용하여 테스트 실행 전 공통 작업을 수행할 수 있다.
- @BeforeEach는 각 테스트 메서드 실행 전에 호출되며, @BeforeAll은 테스트 클래스의 모든 테스트가 실행되기 전에 한 번만 호출된다.
- Trade-off
  - 장점: 테스트 코드의 중복을 줄일 수 있음, 테스트 코드 작성의 편의성 증가
  - 단점: 테스트 코드의 가독성을 떨어트릴 수 있음

### CI(Continuous Integration) : 지속 통합
- 마틴 파울러의 방법론
  1. 모든 것을 버전 관리되는 기본 이력(git main branch 등) : 코드, DB 스키마, 환경 구성 등 모든 부분
  2. 빌드 자동화 : 코드 뿐만 아니라 DB 스키마 반영 등 모든 부분
  3. build self-testing : CI의 필수 조건, 빌드가 스스로 테스트 가능해야 CI임
  4. 모든 기여자가 매일 한번 이상 커밋을 추가
  5. 커밋이 추가될 때마다 빌드를 실행
  6. 빌드가 실패하면 즉시 수정 : 빌드 복구를 최우선 작업
  7. 빠른 빌드 유지 : 지속 통합의 큰 이점은 빠른 피드백 유지를 위함
  8. 진행 중인 작업은 외부로 공개 X : 일반적으로 하나의 기능은 복수의 커밋으로 구현 - 일부 커밋이 추가되어도 완료되지 않은 작업은 노출 X
  9. 운영환경을 모방한 환경에서 테스트 : IAC(Infrastructure as Code)로 운영환경을 모방한 환경에서 테스트
  10. 팀의 모두가 진행상황을 확인 가능 : 시스템 변경의 정보에 모두 접근해야함
  11. 코드 배치(배포) 자동화 : 빈번한 배포가 발생하기 때문에 자동화 필요
